...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        if len(all_certain_junctions_in_remaining_ambulance_path) < 2:
            return 0
        curr_path = [all_certain_junctions_in_remaining_ambulance_path[0]]
        sorted(all_certain_junctions_in_remaining_ambulance_path, key=lambda junction: junction.index)
        curr_remaining = all_certain_junctions_in_remaining_ambulance_path[1::]
        sum = 0
        while len(curr_path) != len(all_certain_junctions_in_remaining_ambulance_path):
            junc_to_append = curr_remaining[0]  # initilization
            min_dist = self.cached_air_distance_calculator.get_air_distance_between_junctions(curr_path[-1],
                                                                                              junc_to_append)
            for junc in curr_remaining:
                dist = self.cached_air_distance_calculator.get_air_distance_between_junctions(curr_path[-1], junc)
                if dist < min_dist:
                    min_dist = dist
                    junc_to_append = junc
            curr_path.append(junc_to_append)
            curr_remaining.remove(junc_to_append)
            sum += dist
        replace_with_tuple = lambda junction: (junction, junction.index)
        curr_path = [replace_with_tuple(junction) for junction in curr_path]
        return sum
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        edges = [(i, j) for i in junctions for j in junctions if i.index < j.index]
        # g.add_weighted_edges_from(edges, weight=lambda
        #     edge: self.cached_air_distance_calculator.get_air_distance_between_junctions(edge[0], edge[1]))
        for edge in edges:
            g.add_edge(edge[0], edge[1],
                       weight=self.cached_air_distance_calculator.get_air_distance_between_junctions(edge[0], edge[1]))
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        def air_dist_to_closest_lab(junction: Junction) -> float:
            ...
            """
            return min(dist_func(junction, lab.location) for lab in labs)

        waiting_apartments = self.problem.get_reported_apartments_waiting_to_visit(state)
        nr_tests_on_ambulance = state.get_total_nr_tests_taken_and_stored_on_ambulance()
        curr_cost = nr_tests_on_ambulance * air_dist_to_closest_lab(state.current_location)
        waiting_cost = sum(
            apartment.nr_roommates * air_dist_to_closest_lab(apartment.location) for apartment in waiting_apartments)
        return curr_cost + waiting_cost
