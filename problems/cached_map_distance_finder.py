...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        """
        src_id = src_junction.index
        tgt_id = tgt_junction.index
        cache_key: Tuple[int, int] = (src_id, tgt_id)
        if self._is_in_cache(cache_key):
            return self._get_from_cache(cache_key)
        else:
            new_problem: MapProblem = MapProblem(self.streets_map, src_id, tgt_id)
            search_result = self.map_problem_solver.solve_problem(new_problem).solution_cost
            self._insert_to_cache(cache_key, search_result)
