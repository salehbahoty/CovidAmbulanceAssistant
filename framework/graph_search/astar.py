...
class AStar(BestFirstSearch):
    ...
    def _calc_node_expanding_priority(self, search_node: SearchNode) -> float:
        ...
        """
        return (self.heuristic_weight * self.heuristic_function.estimate(search_node.state)) + (
    ...
    def _open_successor_node(self, problem: GraphProblem, successor_node: SearchNode):
        ...
        if not self.open.has_state(successor_node.state):
            if self.close.has_state(successor_node.state):
                node_founded_in_close = self.close.get_node_by_state(successor_node.state)
                if node_founded_in_close.g_cost > successor_node.g_cost:
                    self.close.remove_node(node_founded_in_close)
