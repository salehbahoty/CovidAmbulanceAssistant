    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                mid_w = 1.0 * (high_heuristic_weight + low_heuristic_weight) / 2
                astar_solver = AStar(heuristic_function_type=self.heuristic_function_type,
                                     max_nr_states_to_expand=self.max_nr_states_to_expand_per_iteration,
                                     heuristic_weight=mid_w)
                res = astar_solver.solve_problem(problem)
                ...
                if not res.is_solution_found:
                    low_heuristic_weight = mid_w
                else:
                    high_heuristic_weight = mid_w
                    if res.solution_g_cost < best_solution.solution_g_cost:
                        best_solution = res
                        best_heuristic_weight = mid_w
