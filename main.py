...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    Use `matplotlib` to generate a figure of the distance & #expanded-nodes
     w.r.t. the weight.
     [Ex.15]: Complete the implementation of this method.
    """
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    weights_arr = np.linspace(0.5, 0.95, 30)
    cost_list = []
    nr_expanded_states_list = []
    used_weight_list = []
    for w in weights_arr:
        astar_for_given_w = AStar(heuristic_type, w, max_nr_states_to_expand)
        res = astar_for_given_w.solve_problem(problem)
        if res.is_solution_found:
            cost_list.append(res.solution_g_cost)
            nr_expanded_states_list.append(res.nr_expanded_states)
            used_weight_list.append(w)
    plot_distance_and_expanded_wrt_weight_figure("best weighted A*", used_weight_list, cost_list,
                                                 nr_expanded_states_list)
...
def toy_map_problem_experiments():
    ...
    astar_null_heuristic = AStar(NullHeuristic)
    res = astar_null_heuristic.solve_problem(toy_map_problem)
    print(res)

    # Ex.13
    #  create an instance of `AStar` with the `AirDistHeuristic`,
    #       solve the same `toy_map_problem` with it and print the results (as before).
    astar_air_dist_heuristic = AStar(AirDistHeuristic)
    res = astar_air_dist_heuristic.solve_problem(toy_map_problem)
    print(res)
...
def mda_problem_with_astar_experiments():
    ...
    astar_mda_max_air_dist_heiristic = AStar(MDAMaxAirDistHeuristic)
    res = astar_mda_max_air_dist_heiristic.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)

    # Ex.25
    # create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astar_mda_sum_air_dist_heiristic = AStar(MDASumAirDistHeuristic)
    res = astar_mda_sum_air_dist_heiristic.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)

    # Ex.28
    # create an instance of `AStar` with the `MDAMSTAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astar_mda_mst_air_dist_heiristic = AStar(MDAMSTAirDistHeuristic)
    res = astar_mda_mst_air_dist_heiristic.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
...
def monetary_cost_objectives_mda_problem_experiments():
    ...
    # Ex.32
    #  create an instance of `UniformCost`
...
def multiple_objectives_mda_problem_experiments():
    ...
    astar_test_travel = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = astar_test_travel.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
    ...
    #       Solve the `moderate_mda_problem_with_tests_travel_dist_cost` with it and print the results.
    eps = 0.6
    astar_A_2 = AStar(MDAMSTAirDistHeuristic)
    res = astar_A_2.solve_problem(moderate_mda_problem_with_distance_cost)
...
def mda_problem_anytime_astar_experiments():
    ...
    anytime_astart = AnytimeAStar(heuristic_function_type=MDAMSTAirDistHeuristic,
                                  max_nr_states_to_expand_per_iteration=1000)
    res = anytime_astart.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
